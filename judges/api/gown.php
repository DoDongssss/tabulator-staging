<?php
    ...
    if(isset($_GET['getCandidate'])){
        $sql = "SELECT c.id, c.image, c.number
            FROM contestant c
            LEFT JOIN gown p ON c.id = p.contestant_id 
    ...
    if(isset($_POST['candidateId']) && $_POST['judgeId']){
        ...
        $sql = "INSERT INTO `gown`( `judge_id`, `contestant_id`,`beauty_of_the_face`, `elegance`, `poise_bearing_confidence`, `overall_stage_appeal`, `total`) VALUES ('$judgeId','$candidateId','$score1','$score2','$score3','$score4', '$total')";
        $result = $con->query($sql);
