...
function getCandidate(){
    $.ajax({
        url: "api/playsuit.php?getCandidate=1",
        method: "GET",
        dataType: 'JSON',
        success: function(response){
            if(response.status){
                ...
                window. location. href = '/tabulation/judges/interview.php'
            }
        },
        error: function(err){
...
const submitPlaysuit = document.getElementById('submitPlaysuit');

submitPlaysuit.addEventListener('submit', (e) => {
    e.preventDefault();
    
    $.ajax({
        url: "api/playsuit.php",
        method: "POST",
        dataType: 'json',
        data:{
            judgeId : 1,
